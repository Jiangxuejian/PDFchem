...
!INQUIRE(FILE="avpdf_input.dat", EXIST=file_exists)
!if (file_exists) then
  ! Check if the user passed two arguments
...
open(20, file=output_file, status='replace', iostat=ios)
if (ios /= 0) then
  !write(*,*) "Error: Cannot open output file."
  open(unit=20,file='output_f90.dat',status='replace')
  !stop
endif

!do ipref = 1,1 !No. PDR simulations
do ipref = 1,2501 !No. PDR simulations
  prefix = pref(ipref)
  ...
  !write output.dat
  write(20,'(100ES11.2)') fuv, cosmicrays, Z, & !parameters of PDR simulation
...
subroutine readpdf  
  ...
  avtot = 0
  do
    read(1,*,end=1000) dummy
    avtot = avtot+1
  enddo
  1000 continue
  rewind(1)
  allocate(avin(1:avtot),pdfin(1:avtot))
  do i=1,avtot
    read(1,*,iostat=IERR) avin(i),pdfin(i)
    if (IERR/=0) continue
  enddo
  !avin = 10**avin
  !pdfin = 10**pdfin
  ...
  !integer :: IERR = 0
  !close(1);open(unit=1,file='avpdf_input.dat',status='old')
  !avtot = 0
  !do
  !  read(1,*,end=1000) dummy
  !  avtot = avtot+1
