def main():
    ...
    pref = makeprefix()
    with open('output_py.dat', 'w') as outfile:
            ...
            for i in range(itot-1):
                ...
                for j in range(0, 17):
                    if pop[j,1,i] == 0 or abs(g[j,1]*pop[j,0,i]/pop[j,1,i]/g[j,0]-1) < 1e-2:
                        ...
                        Tex[j,i] = (hp*freq0[j]/kb)/math.log(g[j][1]*pop[j,1,i]/pop[j,1,i]/g[j][0]) # excitation temperature
                ...
                phi = 1./sigma/math.sqrt(2.*math.pi) # phi value used in optical depth
