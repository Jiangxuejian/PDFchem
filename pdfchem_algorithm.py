...
def readfile(directory, prefix, Nspec):
    ...
    #with open('variable_density.dat', 'r') as f: # variable density slab
    #    line = file.readline().split()
    #    x[i] = float(line[0])
    #x = np.loadtxt('variable_density.dat')  # variable density slab
...
# Av,obs - PDF function
def pdf(x, s, m):
    return (1.0 / (s * math.sqrt(2.0 * np.pi))) * math.exp(-(math.log(x) - m) ** 2 / (2.0 * s ** 2))
...
def makeprefix():
    num = [str(i).zfill(2) for i in range(41)]
    pref = []
    for i in range(41):
        for j in range(41):
...
with open('output.dat', 'w') as outfile:
    for ipref in range(0, 1681): # No. PDR simulations
        screen_time = datetime.datetime.now().strftime("\033[0;32m%H:%M:%S %Y-%m-%d > \33[0m")
        print(f"\r{screen_time} Looping through {ipref+1}/{1681} Simulations", end='', flush=True)
        prefix = pref[ipref]
        ...
        for i in range(itot-1):
            ...
            for j in range(0, 17):
                if pop[j,1,i] == 0 or abs(g[j,1]*pop[j,0,i]/pop[j,1,i]/g[j,1]-1) < 1e-2:
                    Tex[j,i] = 0
                    Bnu[j,i] = 0
                else:
                    Tex[j,i] = (hp*freq0[j]/kb)/np.log(g[j][1]*pop[j,1,i]/pop[j,1,i]/g[j][0]) # excitation temperature
                    Bnu[j,i] = (2.*hp*freq0[j]**3/c**2)/(np.exp(hp*freq0[j]/kb/Tex[j,i])-1) # Black body emission
            sigma = (freq0/c)*np.sqrt(kb*tgas[i]/mh+vturb**2/2.) # sigma value used in phi
            phi = 1./sigma/np.sqrt(2.*np.pi) # phi value used in optical depth
            frac = 0.5*((pop[:,0,i]+pop[:,0,i+1])*g[:,1]/g[:,0]-(pop[:,1,i]+pop[:,1,i+1]))
            tau_incr += phi*(A*c**2/8./np.pi/freq0**2)*frac*step # optical depth calculation
            tau[:,i] = tau_incr # record value
            
        # solve radiative transfer equation
        Ntr, Ntot, t_r, Ncol = 0, 0, np.zeros(17), 0
        tau_cii, tau_ci, tau_co = 0, 0, 0
        for i in range(itot-2):
            Avobs = 0.06*(nh[i]**0.69) # Av,obs -- nH relation
            for k in range(1, avtot+1):
                if (avin[k-1] > Avobs):
                    break
            dtau = tau[:, i+1] - tau[:, i]
            for j in range(17): # frequencies explored (see subroutine constants)
                if (dtau[j] > 1e10):
                    ...
                    t_a_i = Bnu[j, i]*((1-np.exp(-dtau[j]))/dtau[j]-np.exp(-dtau[j]))
                    t_a_ip1 = Bnu[j, i+1]*(1-(1-np.exp(-dtau[j]))/dtau[j])
                    t_r[j] = t_r[j]*np.exp(-dtau[j])+t_a_i+t_a_ip1
                else:
                    t_r[j] = t_r[j]*(1-dtau[j])+(Bnu[j, i]+Bnu[j, i+1])*dtau[j]/2
            step = abs(x[i]-x[i+1])*pc2cm
            Ntot = Ntot + pdfin[k-1]
            Ntr = Ntr + (t_r*c**2/2/kb/freq0**2)*pdfin[k-1]
            Ncol = Ncol + (0.5*(nh[i]+nh[i+1])*step)
            tau_cii = tau_cii + dtau[0]*pdfin[k-1]
            tau_ci = tau_ci + dtau[1]*pdfin[k-1]
            tau_co = tau_co + dtau[7]*pdfin[k-1]

        # write output.dat
        outfile.write('{:100e}{:100e}{:100e}'.format(fuv, cosmicrays, Z))  #!parameters of PDR simulation
        for i in range(33):
            outfile.write('{:100e}'.format(N[i]/N[0]))
        outfile.write('{:100e}{:100e}{:100e}'.format(Ntr[0], Ntr[1], Ntr[3]))
        for i in range(7, 17):
            outfile.write('{:100e}'.format(Ntr[i]))
        outfile.write('\n')
